...
class RunnerTests(unittest.TestCase):
    def test_report_retries_only_bounded_windows_access_failures(self):
        error=PermissionError('transient Windows sharing denial');error.winerror=5
        with scratch_case() as root:
            path=root/'report.json';path.write_text('{"old": true}')
            calls=[]
            def transient(source,target):
                ...
                if len(calls)<3:
                    ...
                    raise error
                # End the injected sequence without involving Windows here.
                # A real sharing denial during this mocked call would add an
                # uncontrolled retry to the exact backoff assertion below.
                return target
            with mock.patch.object(pathlib.Path,'replace',transient), mock.patch.object(runner.time,'sleep') as sleep:
                runner.atomic_json(path,{'new':True})
            self.assertEqual(json.loads(path.read_text()),{'old':True})
            self.assertEqual(json.loads(path.with_suffix('.tmp').read_text()),{'new':True})
            self.assertEqual(len(calls),3)
            self.assertEqual([c.args[0] for c in sleep.call_args_list],[0.02,0.04])
            # Independently exercise the real atomic replacement and its real
            # bounded retry policy, without counting OS timing as mock output.
            runner.atomic_json(path,{'new':True})
            self.assertEqual(json.loads(path.read_text()),{'new':True})
            with mock.patch.object(pathlib.Path,'replace',side_effect=error) as replace, mock.patch.object(runner.time,'sleep'):
                with self.assertRaises(PermissionError):runner.atomic_json(path,{'failed':True})
                self.assertEqual(replace.call_count,6)
            self.assertEqual(json.loads(path.read_text()),{'new':True})
            with mock.patch.object(pathlib.Path,'replace',side_effect=PermissionError('ordinary access failure')) as replace:
                with self.assertRaises(PermissionError):runner.atomic_json(path,{})
    ...
    def run_steps(self, steps, **options):
        with scratch_case() as root:
            ...
            try:
                with contextlib.redirect_stdout(io.StringIO()):
                    code = runner.execute({'timeoutSeconds': 10, **options}, steps, 'self-test')
    ...
    def test_independent_checks_continue_but_failed_dependencies_do_not(self):
        code, report, xml, logs, _ = self.run_steps([
            self.step('fail', "raise SystemExit(7)"),
            self.step('dependent', "raise AssertionError('must not execute')", requires=['fail']),
            self.step('independent', "print('independent completed')"),
            self.step('transitive', "raise AssertionError('must not execute')", requires=['dependent'])
        ], continueIndependent=True)
        self.assertEqual(code, 1)
        self.assertEqual([s['status'] for s in report['steps']], ['failed', 'not_run', 'passed', 'not_run'])
        self.assertEqual((xml.attrib['failures'], xml.attrib['skipped']), ('1', '2'))
        self.assertIn('independent completed', logs[-1])
