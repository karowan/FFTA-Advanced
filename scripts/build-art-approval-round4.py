"""Complete review with native-derived face anchors and front-facing badges."""
...
BASE=ROOT/'build/art/job-art-approval-v3-2026-09-20'
...
TEMPLATE=ROOT/'scripts/art-approval-round4.html.txt'
def main():
    ...
    results=json.loads((GEN/RESULTS_NAME).read_text());assert len(results['assets'])==12
    files=[]
    def copy(r,name):
    ...
    for u,first in zip(data['units'],prior['units']):
        ...
        for a in [x for x in results['assets'] if x['job']==u['job']]:
            kind=a['kind'];prefix=slug+'-round'+str(ROUND_NUMBER)+'-'+kind
            ...
            if kind=='portrait':u.update(newPortrait=native,newPortraitSource=sampled)
                ...
                u['cleanRaceIcons']=copy(a['anchors']['referenceStrip'],slug+'-clean-race-icons')
                layer=GEN/slug/'final-anchor-layer.png'
                layerrec=dict(path=str(layer.relative_to(ROOT)),sha256=sha(layer.read_bytes())) if layer.exists() else a['anchors']['layer']
                u['anchorLayer']=copy(layerrec,slug+'-anchor-layer')
                u['anchorProof']=a['anchorProof'];u['anchorMode']=a['anchorMode']
                u['anchorCoordinates']=a.get('conversionAnchors',a['anchors'])['coordinates']
                u['paletteDecision']=a.get('paletteDecision','Existing badge palette retained.')
                ...
                for state,path in first['badges'].items():
                    ...
                    palette_source=Image.open(FIRST/'assets'/(a['badgePaletteSourceSlug']+'-badge-'+state+'.png')) if 'badgePaletteSourceSlug' in a else im
                    out=Image.fromarray(new);out.putpalette(palette_source.getpalette());dest=OUT/'assets'/(prefix+'-'+state+'.png');out.save(dest)
                    if state=='eligible':assert out.getpalette()[:48]==Image.open(OUT/native).getpalette()[:48]
    ...
    data.update(round=ROUND_NUMBER,status='Native-palette portrait and badge proposals await user review. No ROM changes.')
    ...
    template=add_zoom(TEMPLATE.read_text(encoding='utf-8'))
    ...
    board=Image.new('RGBA',(1000,620),(231,229,223,255));draw=ImageDraw.Draw(board)
    for n,u in enumerate(data['units']):
        x=n%5*200;y=n//5*200;draw.text((x+3,y+3),u['label'],fill='black')
    ...
    for n,u in enumerate([u for u in data['units'] if u['eyeRevised']]):
        im=Image.open(OUT/u['newPortrait']).convert('RGBA');board.alpha_composite(im.resize((144,168),Image.Resampling.NEAREST),(n*200+550,420))
