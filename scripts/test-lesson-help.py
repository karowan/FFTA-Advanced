"""Native original/gear help preservation and implemented lesson descriptions."""
...
P=ROOT/'build/expansion/probes';meta=json.loads((P/'combat.json').read_text());rom=(P/'combat.gba').read_bytes()
assert hashlib.sha1(rom).hexdigest()==meta['romSha1'];help=meta['help'];base=(P/'content-data.gba').read_bytes()
...
(P/'lesson-help-tests.json').write_text(json.dumps(result,indent=2));print(json.dumps(result,indent=2))
