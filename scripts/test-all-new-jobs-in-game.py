...
if integrated:
 manifest=json.loads((ROOT/'build/expansion/probes/integrated-jobs/current.json').read_text())
 data=pathlib.Path(manifest['path']).read_bytes()
 OUT=pathlib.Path(manifest['path']).parent/'all-new-jobs-ui';OUT.mkdir(exist_ok=True)
else:
...
engine=(OUT/'engine.bin' if frozen and not integrated else ROOT/'build/expansion/engine.bin').read_bytes()
symbols=(OUT/'engine.symbols' if frozen and not integrated else ROOT/'build/expansion/engine.symbols').read_text()
...
exec(compile(ast.Module(body=[n for n in tree.body if isinstance(n,(ast.ClassDef,ast.FunctionDef)) and n.name in ('ARM','iwram_from_boot')],type_ignores=[]),'<native ARM>','exec'))
if integrated:
 fix=pathlib.Path(manifest['path']).parent/'fixture'
 proof=json.loads((fix/'prepare-cache.json').read_text())
 assert (fix/'frozen.gba').read_bytes()==data
 iwram=(fix/'battle-ready.iwram').read_bytes()
 assert hashlib.sha1(iwram).hexdigest()==proof['outputs']['battle-ready.iwram']
else:iwram=iwram_from_boot()
machine=ARM(data,iwram);machine.get=machine.read;machine.r32=machine.word
machine.r16=lambda p:int.from_bytes(machine.read(p,2),'little')
def labels():
...
report={'passed':True,'romSha1':hashlib.sha1(data).hexdigest(),'engineSha1':None if integrated else manifest['engineSha1'],
