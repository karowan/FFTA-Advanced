...
ACTION=int(sys.argv[1]);case=OUT/('medicine-turns-20260917T022536.750669Z/'+str(ACTION)+'-useful-5')
assert meta['romSha1']=='12f790448f2fc95e259b78a403895ce0320468d2'
...
if '--discover' in sys.argv:
 (OUT/('medicine-ai-discovery-'+str(ACTION)+'.json')).write_text(json.dumps(dict(romSha1=meta['romSha1'],discovery=discovery),indent=2))
...
filter_trace=[];tracking=False
def track_filter(u,pc,size,data):
 global tracking
 if pc==0x080c359c:
  p=m.word(u.reg_read(UC_ARM_REG_SP)+12);tracking=0x02000000<=p<=0x0203ffec and int.from_bytes(m.read(p,2),'little')==ACTION
 if tracking:filter_trace.append(dict(pc=hex(pc),r=[hex(u.reg_read(reg)) for reg in (UC_ARM_REG_R0,UC_ARM_REG_R1,UC_ARM_REG_R2,UC_ARM_REG_R3)]))
 if pc in (0x080c478c,0x080c37b4):tracking=False
m.u.hook_add(UC_HOOK_CODE,track_filter,begin=0x080c359c,end=0x080c478c)
...
report=dict(diagnostic=True,filterTrace=filter_trace,discovery=discovery,romSha1=meta['romSha1'],sourceState=str(case/'AI-turn-start.state'),actor=actor_info,groups=groups,nodes=nodes,events=events,selected=m.read(0x02015488,44).hex())
(OUT/('medicine-ai-trace-'+str(ACTION)+'.json')).write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
