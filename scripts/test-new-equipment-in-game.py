...
import datetime, hashlib, importlib.util, json, pathlib, struct, sys
...
ABILITIES='--abilities' in sys.argv or INTEGRATED
ROM=OUT/('ability-core.gba' if ABILITIES else 'content-inventory.gba')
if INTEGRATED:
    meta=json.loads((OUT/'integrated-jobs/current.json').read_text())
    ROM=pathlib.Path(meta['path'])
    assert hashlib.sha1(ROM.read_bytes()).hexdigest()==meta['romSha1']
    OUT=ROM.parent/('opening-teaching-'+datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%S.%fZ'))
    OUT.mkdir()
...
report={'passed':True,'romSha1':hashlib.sha1(ROM.read_bytes()).hexdigest(),'checks':checks,
        ...
        'remaining':(['Campaign earning of original progression gates (V06)','Final assembled acceptance (R02)'] if INTEGRATED else
                     ['Axe artwork/battle animations','Equipment AP learning/commands/effect integration',
