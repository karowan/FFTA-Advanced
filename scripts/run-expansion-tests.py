...
def atomic_json(path, data):
    ...
    # Windows readers (including indexing/antivirus) may briefly deny atomic
    # replacement. Keep the previous complete report and retry the rename,
    # never delete the destination or suppress a persistent access failure.
...
def execute(plan, steps, selection):
    ...
    # Refuse before locking: a machine already short of disk or memory must
    # not accept another gigabyte-scale run (notes/resource-incident-2026-09-15.md).
    admission = resource_guard.require_admission(ROOT, 'expansion tests')
    with resource_guard.WorkspaceLock(RUNS / 'runner.lock'):
        stamp = dt.datetime.now(dt.timezone.utc).strftime('%Y%m%dT%H%M%S.%fZ')
        output = RUNS / stamp
        output.mkdir()
        report = dict(schema=1, status='running', selection=selection,
                      scope=plan.get('scope', 'Declared implemented-feature regression only; not expansion completion'),
        ...
        for i, (step, result) in enumerate(zip(steps, report['steps'])):
            blocked_by = [name for name in step.get('requires', [])
                          if results_by_id[name]['status'] != 'passed']
            if blocked_by:
                result['detail'] = 'Prerequisites did not pass: ' + ', '.join(blocked_by)
                atomic_json(output / 'report.json', report)
                continue
            ...
            state, problems = resource_guard.admission_problems(ROOT)
            result['resourcesBefore'] = state
            if problems:
                result.update(status='error', detail='System pressure before step: ' + '; '.join(problems))
                result['seconds'] = 0.0
                print(f'  {result["status"]}: {result["detail"]}', flush=True)
                atomic_json(output / 'report.json', report)
                break
            try:
                # The step and all of its descendants run in one contained,
                # below-normal-priority process tree with a memory ceiling; a
                # timeout terminates the whole tree, never just the parent.
                with log.open('wb') as stream:
                    code, peak = resource_guard.run_contained(
                        command, step.get('timeoutSeconds', plan['timeoutSeconds']),
                        cwd=ROOT, env=environment, stdout=stream, stderr=subprocess.STDOUT)
                if peak is not None:
                    result['peakMemoryMB'] = round(peak / 1024 ** 2, 1)
                if code is None:
                    result.update(status='timed_out', detail='Declared time limit exceeded; process tree terminated')
                else:
        ...
        try:
            # Retention: keep every report and log, drop stale bulk outputs so a
            # run cannot leave the volume fuller than it found it.
            import importlib.util
            spec = importlib.util.spec_from_file_location('prune_build_outputs', ROOT / 'scripts/prune-build-outputs.py')
            pruner = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(pruner)
            report['pruned'] = pruner.prune()
            report['resourcesAfter'] = resource_guard.snapshot(ROOT)
        except Exception as error:  # Retention must never turn a passed run into a failure.
            report['pruneError'] = repr(error)
        atomic_json(output / 'report.json', report)
