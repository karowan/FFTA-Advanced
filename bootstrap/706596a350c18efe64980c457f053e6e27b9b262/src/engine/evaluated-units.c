...
static int ram_container(uintptr_t p) {
    return !(p&3) && p>=0x02000000u && p<=0x0203f800u-sizeof(FFTA_EvaluatedUnit);
}
...
static int heap_container(uintptr_t p) {
    ...
    if ((base&3) || base<0x02000000u || base>0x0203f7f8u) return 0;
    uintptr_t end=base+8u+4u*half((const uint8_t *)base+6);
    if (end<=base+8 || end>0x0203f800u || p<base+20 || p+sizeof(FFTA_EvaluatedUnit)>end) return 0;
