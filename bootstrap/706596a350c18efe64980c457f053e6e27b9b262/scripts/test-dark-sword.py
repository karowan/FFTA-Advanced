...
for action,removed,undead,hp,mp,target_mp in itertools.product((357,358,346),(0,1,2,4,5,9,10,79,80,99,999),(0,1),(1,50,100),(0,47,50),(0,3,49)):
 actor=bytearray(264);target=bytearray(264);struct.pack_into('<HHHH',actor,0x18,hp,100,mp,50);struct.pack_into('<H',target,0x1c,target_mp);struct.pack_into('<H',target,0x28,0x8000);target[0xe8]=4*undead
...
def execute(m,action,hp=250,mp=49,effect=0,undead=0,seed=0,residue=0):
 ...
 m.put(TARGET+0xe8,bytes([(ram[TARGET-0x02000000+0xe8]&~4)|4*undead]));m.put(items+389*32+26,bytes([effect,0,0]))
