...
ROOT=pathlib.Path(__file__).resolve().parents[1];FIX=ROOT/'build/expansion/probes/battle-fixture';meta=json.loads((FIX/'report.json').read_text());rom=(FIX/'frozen.gba').read_bytes();assert hashlib.sha1(rom).hexdigest()==meta['romSha1']
assert meta['romSha1']==json.loads((ROOT/'build/expansion/probes/combat.json').read_text())['romSha1'], 'Stale battle fixture'
OUT=ROOT/'build/expansion/probes/combos-in-game'/meta['romSha1'];OUT.mkdir(parents=True,exist_ok=True);ROM=OUT/'frozen.gba';ROM.write_bytes(rom);START=OUT/'battle-ready.state';START.write_bytes((FIX/'battle-ready.state').read_bytes());registry=json.loads((ROOT/'build/expansion/registry.json').read_text());Emulator=runpy.run_path(str(ROOT/'scripts/emulator-test.py'))['Emulator']
...
for lesson in registry['lessons']:
 ...
 for owner in lesson['owners']:
  run(lesson,owner);chain(lesson,owner)
report={'passed':True,'romSha1':meta['romSha1'],'checks':sum(checks.values()),'groups':checks,'owners':results,'chains':chains,'scope':'Actual native Combo UI cancellation/commit for each tested owner; new lesson as an actual participant with exact named-donor control; native Save Now and SRAM-only cold Resume Battle. Disposable legal job/equipment/mastery/JP/targetHP fixtures, no user files.'}
(OUT/'results.json').write_text(json.dumps(report,indent=2));
if '--lesson' not in sys.argv:(ROOT/'build/expansion/probes/combos-in-game-tests.json').write_text(json.dumps(report,indent=2))
