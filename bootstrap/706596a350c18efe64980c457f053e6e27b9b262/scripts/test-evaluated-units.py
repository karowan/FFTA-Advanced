...
for residue in (0,4):
 ...
 p=call('ffta_evaluated_allocate',264,stack=STACK+residue);check('actual allocated size',a.call(0x0800717c,0,p),276)
 ...
 before=a.read(0x03007300,276);check('null init fails',call('ffta_evaluated_init',0x03007300,0),0);check('failed init unchanged',a.read(0x03007300,276),before)
 ...
 recycled=a.call(0x08022840,276);check('unregistered reuse rejected',address(recycled),0)
...
def outer(m,action,residue,kind=15,heapsize=None):
 ...
 def inspect(u,pc,size,data):
  ...
  events.append((actor,target,m.read(actor+264,12),m.read(target+264,12)))
