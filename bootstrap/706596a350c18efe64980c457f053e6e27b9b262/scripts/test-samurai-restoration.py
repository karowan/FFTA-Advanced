...
for action,actor_side,target_side,charm,confusion,hp,undead in itertools.product((350,351),(0,1),(0,1),(0,1),(0,1),(0,100),(0,1)):
 ...
 expanded.put(TARGET+0xe8,bytes([undead<<2]))
...
for packed,residue in itertools.product(range(256),(0,4)):
 reset(expanded,state=packed);expanded.put(TARGET+0x18,struct.pack('<HH',1,999));expanded.put(TARGET+0xe8,b'\x04')
...
for packed,seed in itertools.product((1,5,13),(0,1)):
 reset(expanded,state=packed);expanded.put(TARGET+0x18,struct.pack('<HH',100,250));expanded.put(TARGET+0xe8,b'\x04');expanded.put(0x030034b0,struct.pack('<I',seed))
