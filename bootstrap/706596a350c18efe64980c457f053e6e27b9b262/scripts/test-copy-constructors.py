...
for stack in (STACK, STACK - 4):
    ...
    for generation in range(3):
        snapshot = m.call(0x08022840, 0x1014)
        check('allocation', m.call(0x0800717c, 0, snapshot) == 0x1014, 'snapshot capacity')
        ...
        for position, slot in enumerate(reversed(range(13))):
            ...
            check('snapshot_rows', address == snapshot + 0xe24 + position * 38, 'tail identity')
...
for stack in (STACK, STACK - 4):
    ...
    check('manager', m.call(0x0800717c, 0, manager) == 0x400, 'manager capacity')
    check('manager', m.read(manager + 0x3b4, 76) == bytes(76), 'tail not initialized')
    check('manager', tail(m, manager + 0x40) == manager + 0x3b4, 'first manager copy')
    check('manager', tail(m, manager + 0x148) == manager + 0x3da, 'second manager copy')
    ...
    check('selection', m.call(0x0800717c, 0, selection) == 0x3828, 'selection allocation size')
    check('selection', m.read(selection + 0x3800, 38) == bytes(38), 'selection tail')
...
for start in (0x020159d0, 0x0201f550, 0x0200f3c4):
    for adjust in (0, -4):
        ...
        m.call(symbols['ffta_on_unit_clear'], start, 0x0203f800 - start + adjust)
...
if 'ffta_party_copy_register' in symbols:
    for mode in (0, 1):
        for stack in (STACK, STACK - 4):
            ...
            check('party', m.call(0x0800717c, 0, party) == 0x7268, 'party allocation')
            check('party', m.word(0x0203ff40) == party, 'party registration')
            check('party', tail(m, preview) == address, 'preview ownership')
            check('party', m.read(address, 38) == bytes(38), 'party tail initialization')
            ...
            for slot, (native, ap, preference) in enumerate(original):
                ...
                m.put(0x02001b40 + slot * 34, ap)
            before = m.read(address, 38)
            m.call(0x081443fc, preview, UNIT, 260, stack=stack)
            check('party_copy', m.read(address, 38) == before, 'partial copy touched tail')
            ...
            check('party_clear', m.read(address, 38) == bytes(38), 'full clear retained AP or status')
