"""Deterministic Moon Blossom menu, recipient, hit/miss and Centered execution."""
...
for friendly in (False,True):
 facing=3;blocked=False
 dx,dy,key=((0,1,32),(-1,0,64),(0,-1,16),(1,0,128))[facing]
 d=LAB/f'moon-game-ally{friendly}';d.mkdir(exist_ok=True);e=Emulator(ROM)
 try:
  ...
  e.set_memory(ACTOR+0x2a,struct.pack('<5H',383,0,0,0,0));e.set_memory(0x1b47,b'\xff')
  for turn in range(4):
   previous=active(e)
  ...
  place(e,ACTOR,wrappers[ACTOR],4,6)
  for unit,wrapper,xy in zip(targets,tuple(wrappers[u] for u in targets),((7,6),(6,5),(6,7))):
   place(e,unit,wrapper,*xy);e.set_memory(unit+0x18,struct.pack('<HHHH',999,999,50,50));e.set_memory(unit+0xe8,bytes(8));e.set_memory(unit+0x3a,bytes(2))
  ...
  for k in (256,128,128):tap(e,k)
  tap(e,256,600)
  for k in (256,32,256):tap(e,k)
  capture(e,d,'ability-menu');tap(e,256);capture(e,d,'target-area');preview=capture(e,d,'preview')
  check(half(preview,0xf3fc)==354,('action',half(preview,0xf3fc)));check(preview[AS]==5 and half(preview,ACTOR+0x1c)==50,'preview consumes')
 ...
 for seed in (0,1):
  ...
  for unit in targets:
   ...
   check(damage==base*135*5//400,('native P with Centered',blocked,seed,unit,damage,base))
   if friendly and unit==targets[1]:check(damage==0,'Moon Blossom damaged an ally in the line')
  check(half(r,ACTOR+0x1c)==34 and r[AS]==1,'one MP cost/Centered consumption')
  check(r[0x1940:0x1e70]==before[0x1940:0x1e70],'AP/inventory')
  check(bool(r[ACTOR+0xe8]&8)==any(999-half(r,u+0x18)>0 for u in targets),'Regen requires actual enemy HP damage')
 ...
 if friendly:
  ...
  try:
   ...
   menu(e);r=capture(e,d,'cold-resumed');check(r[AS]==expected[AS],'cold Centered');check(r[ACTOR+0xe8]==expected[ACTOR+0xe8],'cold Regen')
...
report=dict(passed=True,romSha1=meta['romSha1'],checks=checks,outcomes=outcomes,scope='Actual Move and self cross, three targets, ally exclusion, native P, Centered once, damage-dependent Regen, Silence, preview/cancel, turns and cold native suspend resume')
(LAB/'moon-game-report.json').write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
