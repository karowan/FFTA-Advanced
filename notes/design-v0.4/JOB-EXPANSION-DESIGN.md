# FFTA job expansion: proposed roster and progression

Status: design draft, not installed or implemented. **Confirmed scope: our additions on top of the original US FFTA, with no existing overhaul or mod pack.** The complete job specification is now in [JOB-CLASS-SPECIFICATION.md](JOB-CLASS-SPECIFICATION.md); that document takes precedence where it refines this initial outline.

The [version 0.4 design principles](JOB-DESIGN-PRINCIPLES.md) prioritize each job's spirit and useful FFTA gameplay. The [inspiration register](JOB-THEME-AUDIT.md) covers all 116 entries without requiring copied abilities. Source-game mechanics and balance are not binding.
...
All 18 proposed supports now work beyond their teaching jobs; see [support skills and example builds](SUPPORT-SKILL-DESIGN.md). This preserves racial access and one support slot while removing job-set and signature-weapon dependencies.
...
These are proposed identities and example mechanics, not claims of working implementations.
...
| Dark Knight | Spend personal HP for strong attacks; recover through limited draining attacks | Offensive risk/recovery cycle rather than Paladin or Defender protection; must not sustain itself indefinitely without help |
| Viking | FFTA2-inspired axe plundering, thunder magic, and ailment resilience | Now shares the new Axe family; steals gil/accessories/armor and uses Tsunami on any ground, enhanced near water |
...
| Chemist | Ranged use of actual medicines, enhanced potency, and inventory-funded reactions | FFT item expertise with FFV Pharmacology inspiration; no invented offensive or mass-revival recipes |
| Bard | Hurdy-style defensive songs, healing, MP recovery, and anti-undead Requiem | Mostly dependable support; Nameless Song alone uses an explicit random buff pool |
| Dancer | Damaging/debilitating dances, draining Jitterbug, and FFV Sword Dance | Mobile enemy disruption with recognizable dance effects; no automatic turn manipulation |
| Mystic Knight | Persistent elemental/status weapon enchantments and Magic Shell | FFV Spellblade preparation for later Fight attacks; existing Gladiator elemental techniques remain immediate |
...
- Pick AP/equipment learning for this vanilla-based specification. A switch to JP purchasing requires explicit progression and economy adjustments.
