#include <stdint.h>
#ifdef FFTA_PARTY_LIST_OFFSET
extern const uint8_t *const ffta_potion_preference_labels[];
#else
...
static unsigned owned_menu(const uint8_t *ctx,const uint8_t *menu){
#ifdef FFTA_PARTY_LIST_OFFSET
 return ctx && menu==ctx+FFTA_PARTY_LIST_OFFSET;
#else
 (void)ctx;return menu==(const uint8_t *)0x0203c000u;
#endif
}
...
unsigned ffta_potion_roster(unsigned type,unsigned race,uint8_t *menu){
 ...
 if(type!=2 || !ctx || !owned_menu(ctx,menu) || n>=24)return n;
