...
Design version 0.7 — all seven further council additions adopted, September 14, 2026

**Current approved design for our vanilla USA FFTA expansion.** Version 0.7 adds all seven [further council recommendations](ABILITY-EXPANSION-COUNCIL.md) to the adopted 0.5 foundation and [fourteen-action Spellblade](SPELLBLADE-EXPANSION.md). The project now contains 129 abilities and 85 teaching items. New numerical and teaching targets require implementation and gameplay testing. [Design principles](JOB-DESIGN-PRINCIPLES.md) preserve identity and cross-class mixing.

The separate [approved vanilla+ foundation](VANILLA-PLUS-PACKAGE.md) defines completion safeguards, vanilla rare-equipment recovery, clan sorting, Morpher visuals, and menu improvements. It accompanies version 0.7 without revising the class rules or the 85-item teaching plan.
...
The current design contains **129 entries: 85 actions, 18 supports, 18 reactions, and eight combos**. Eight job concepts require ten racial job implementations. Mystic Knight has 14 actions; Chemist has 10; Samurai, Dark Knight, Viking, Geomancer, and Dancer each have nine; Bard has eight. The twelve axe entries are added inside the two existing command sets. These counts describe lessons, not proven available ROM records.
...
**rN** is Manhattan tile distance; r1 excludes diagonals. **Cross** is center plus four orthogonal neighbors. **Self cross** centers on the user. **Line N** is a selected cardinal line excluding the user and stopping at impassable terrain. **Any** permits friendly fire. Single-target range has height limit 3, melee limit 2; cross neighbors are within 2 height of center and line targets within 2 of caster. Projectiles require line of sight; songs, dances, and terrain effects do not. Preview all targets and actual accuracy before commitment. Each new action uses one ordinary action and grants no extra action or turn. Passing Step alone permits immediate post-strike movement from the remaining ordinary allowance, capped at two movement points; it grants no extra movement points or Move reset. Other actions grant no immediate movement. Scheduled wound pulses and persistent fields follow their explicit timing.
...
### Additional council effects (0.7)

**Inoculation eligibility:** use exactly the native Cureall curable-status whitelist plus the explicit shared broad-remedy custom statuses. Bind the native portion to the actual vanilla Cureall handler and enumerate/verify its status IDs before implementation; do not guess an all-status immunity mask. The explicit custom portion includes Blade Wound, Challenged, Wisp Exposure, Polka's physical weakening, and Heathen Frolic's magical weakening. Only enemy applications are preventable. Self-inflicted Exposed and Last Resort's linked drawback are never preempted. No KO, uncurable effect, forced displacement, direct CT/turn grant, or paid cost is blocked. Ordinary Slow/Stop-like ailments are covered only if the native Cureall whitelist includes them; this does not grant immunity to direct turn manipulation. Inoculation has no curative effect on existing statuses. Resolve its prevention before Auto-Cureall considers spending stock for any remaining qualifying ailment. Display the verified whitelist to the player.

**Ward and reductions:** The Blackest Night checks an incoming enemy action after prior applicable redirection/mitigation to determine whether positive HP-payable damage remains. If so, consume once and halve eligible direct physical/magical HP damage for that whole action. Do not lower MP loss or sacrifice costs; wholly redirected actions preserve the ward. Snapshot Poise's beneficial-status condition at incoming-action start as usual, before consumption. Ward × Poise gives 0.375 of otherwise eligible damage; original Protect/Shell still use their native formula inputs. This is an implementation contract, not a claim that existing handlers already execute in this order. Spell Parry retains its existing separate consumption rule.

**Wound and movement:** Blade Wound snapshots the ordinary physical P reference before outgoing bonuses and temporary final-damage/interception modifiers. Its two pulses do not repeat damage supports or reactions. Replacement cancels outstanding old pulses and schedules only the replacement's two pulses; do not bank or immediately add a pulse on refresh. Passing Step is one physical action with an optional bounded movement segment after immediate reactions, not a second action. Track actual terrain-cost points already spent this turn. With Move 4, approach 2 → attack → step 2 is legal; approach 4 leaves no finishing step. Declining a step never reopens a consumed Move. All path legality, traps, occupancy and laws still apply.

**Mixing:** Human/Bangaa Dark Arts users can protect allies with any otherwise legal weapon; Human Paladin/Dark Arts retains its native command, not a third White Magic command. A Moogle Chemist may choose Encouragement for new preventive buffs, or Pharmacology for restoration, never both; Nu Mou cannot inherit the Bard support. Inoculation and Guarding Draught count as item actions, and neither produces amplified healing of its own. Nature's Refuge occupancy is not a status for Encouragement or Poise. Reaving's Provoke establishes no Spellweave category; Passing Step establishes Physical. No new lesson transfers between races or grants another support/reaction slot.
...
| SAM-A9 Higanbana | 300 | 8 MP; primary katana; r1 enemy/A; 0.80P non-elemental physical HP damage. Positive HP damage applies Blade Wound unless immune, with no second S roll. Wound deals floor(0.50 × stored P) at the end of each of the target's next two turns; exactly two pulses, no stacking or banked pulses on replacement. Snapshot ordinary physical attack-versus-defense P, including Protect, before outgoing bonuses, temporary final-damage multipliers, and reaction interceptions. Centered and other applicable damage bonuses affect only the immediate hit; Centered is consumed at execution even on miss. Pulses never recheck defenses, trigger reactions, drain, crit, copy procs, or receive damage bonuses; they are damage-over-time, not physical attack actions. Broad remedies cure the wound; KO/Petrify/job change/battle end clear it. Initial action sequences Physical; pulses establish no category. | Thematic redesign: FFXIV Higanbana attack and lingering damage adapted to a removable two-pulse katana wound. [Inspiration][sam14] |
...
Human Dark Knight/Blue Magic and Bangaa Dark Knight/Prayer are legal. Axe Gladiator/Dark Arts or Viking/Dark Arts can use Dark Mind, The Blackest Night, and Last Resort but cannot use sword-gated drain/sacrifice moves while holding an axe. Bloodcasting and Desperation cannot be equipped together.

The Blackest Night adds a weapon-free protective action to both racial lists. Its native HP cost and converted MP cost are additive; the ward cannot protect either payment. It is a beneficial status, not an extra reaction slot. One eligible enemy action consumes it after prior redirection determines positive HP-payable damage; entirely MP-paid actions do not consume it or receive reduced MP loss. No refund or healing follows consumption.
...
| DRK-A9 The Blackest Night | 300 | 10 MP + 10% user max HP; r3 one active ally/self; Sure; weapon-free and usable while Silenced. Grant a dispellable ward: direct physical/magical HP damage from the next successful damaging enemy action ×0.50, covering all eligible components of that action, then consume. Expires at start of caster's next turn if unused; either unit's KO/Petrify/job change or battle end clears it. One same-name ward per recipient; reapplication replaces. Resolve prior redirection and other mitigation first for eligibility: only positive HP-payable damage consumes the ward; entirely MP-redirected damage neither consumes it nor receives reduced MP loss. No benefit against statuses, instant KO, costs, item effects, damage-over-time, or reactions. No healing, refund, retaliation, or extra action. Payment must leave ≥1 HP; Bloodcasting adds converted MP cost to native sacrifice. Sequence unclassified (non-damaging martial protection). | Thematic redesign: FFXIV self/ally barrier adapted to a paid sacrifice for one enemy action of protection, without a refund. [Inspiration][drk14] |
...
| VIK-A9 Provoke | 250 | 6 MP; r3 one enemy/S; weapon-free and usable while Silenced. Apply Challenged until the end of that target's next turn: its direct physical/magical HP damage to recipients other than the challenger ×0.70; damage to challenger unchanged. Evaluate area recipients separately. One challenger per target; new success replaces prior mark. Does not force movement, targeting, Fight, Berserk, or any extra action. Excludes fixed/percentage damage, status chances, items, costs, damage-over-time, and reactions. Broad remedies cure it; either party's KO/Petrify/job change or battle end clears it. Custom harmful status supports ordinary immunity and Opportunist on later attacks, not this non-damaging action. Sequence unclassified. | Original extension: FFIII Viking Provoke inspires an original damage penalty for ignoring the challenger, without forced AI targeting. [Inspiration][vik3] |
...
| GEO-A9 Nature's Refuge | 300 | 12 MP; r3 center/cross legal tiles; Sure placement; weapon-free, no terrain prerequisite. Place Refuge in the same single per-caster field slot as Rime; either cast replaces that caster's existing field. Expires after caster's second subsequent turn or on caster KO/Petrify/job change/battle end. At each incoming enemy action's start, allied grounded occupants receive magical direct HP damage ×0.80 for that action; Float/flying occupants receive none. Leaving loses protection for future actions. Same-name overlapping Refuges do not stack. No physical protection, healing, status immunity, entry trigger, extra move cost, resource recovery, or affinity change. Occupancy is not a status and cannot trigger Encouragement or qualify Poise. Silence-independent; no Reflect/Return Magic/Doublecast. Sequence Magic; no damage or healing to amplify. | Original extension: FFXI Geomancer aid-and-harm identity adapted into stationary magical shelter competing with hostile Rime. [Inspiration][geo11] |
...
Inoculation and Guarding Draught add preventive medicine to both racial lists. Each uses existing stock with atomic two-item payment. Inoculated intercepts qualifying enemy ailments before Auto-Cureall tests remaining eligible ailments; it never erases an existing ailment or self-paid drawback. Guarding Draught supplies ordinary Protect/Shell, not a new stacking reduction. Both are eligible single-target throws and unclassified item actions for Spellweave.
...
| CHM-A9 Inoculation | 300 | One Potion AND one Cureall; 0 MP; r4 one active non-undead ally/self; Sure, projectile LOS/height; weapon-free and Silence-independent. Apply dispellable Inoculated T2: prevent incoming enemy applications in the shared Cureall/Auto-Cureall curable-ailment whitelist, including explicitly integrated broad-remedy custom ailments. Do not cure existing ailments or block KO, uncurable effects, HP damage, theft, displacement, direct CT/turn grants, or self-paid/linked drawbacks. One status; refresh does not stack. Inoculated prevents eligible applications before Auto-Cureall decides whether an item is needed. Validate and consume both ingredients atomically once. Long Throw eligible; Pharmacology cannot alter prevention or duration. First qualifying application to another ally can trigger the user's Encouragement once; self/refresh/interception cannot. No Reflect/Return Magic/Doublecast. Sequence unclassified (item action). | Original extension: FFV preventive mixtures inspire proactive medicine using existing Potion/Cureall supplies; exact recipe and T2 coverage are original. [Inspiration][mixV] |
| CHM-A10 Guarding Draught | 200 | One Potion AND one Soft; 0 MP; r4 one active non-undead ally/self; Sure, projectile LOS/height; weapon-free and Silence-independent. Apply ordinary Protect and Shell together with vanilla duration/removal; no healing, additional damage multiplier, Regen, Haste, or Reflect. Validate both ingredients and consume atomically once. Long Throw eligible; Pharmacology does not enhance buffs. Existing same-name statuses refresh rather than stack; Encouragement follows its single-recipient/action rule for newly applied statuses only, not self or simple refresh. No Reflect/Return Magic/Doublecast. Sequence unclassified (item action). | Thematic redesign: FFV Protect Potion inspires item-funded Protect/Shell; Potion plus Soft is an original recipe, not a canonical property of Soft. [Inspiration][mixV] |
...
Passing Step is the explicit movement exception: approach, strike, resolve immediate reactions, then optionally spend up to two remaining movement points on a preselected route. The completed step ends ordinary movement; no extra points, turn, or action are granted. Pdance and Physical Spellweave sequencing apply. A post-strike step cannot qualify a pre-strike movement bonus retroactively.
...
| DNC-A9 Passing Step | 250 | 6 MP; r1 one enemy/A; weapon-free; 0.95 Pdance non-elemental physical HP damage. After the strike and immediate enemy reactions, a surviving actor able to move may follow a preselected optional legal path costing at most two remaining normal movement points. Deduct movement already spent this turn; grant no new points. The step is available on hit or miss, uses normal terrain/height/trap/occupancy rules, and ends the ordinary Move opportunity. Immobilize prevents the step but not the strike; KO/Petrify prevents it. Invalidated routes cancel, never teleport or silently reroute. The two-point segment cap is unchanged by Move bonuses; such bonuses only affect the original turn allowance. Declining the step leaves ordinary movement rules intact, never reopens a used Move. No retroactive movement bonus on the preceding hit, second action, or Move reset. Works while Silenced; no Reflect/Return Magic/Doublecast. Sequence Physical. | Original extension: Dancer movement tradition adapted to approach-strike-retreat within the original movement budget, not extra movement. [Inspiration][dnc14] |
...
**Spellblade — Viera.** All 14 actions require a primary weapon of any type and are blocked by Silence (revised September 24, 2026; previously rapier/saber only). Only the eleven enchantments (MYK-A1–A11) offer self preparation (Sure) or one enemy within the weapon's own Fight range (previously r1) for an immediate enchanted A-type strike. Pay once; apply/replace the enchantment even if the strike misses; this is one action, not a free second action or movement. Their strike is physical and not Fight: no crit, weapon proc, second weapon, Doublecast, or automatic bypass of applicable defensive reactions. Spellbreak, Arcane Release, and Break Blade follow their own targeting and resolution below; they never automatically enchant the weapon.
...
The original W1–W8 tables supply 64 job weapons; six Mystic Knight sabers, seven further council teaching weapons, and eight Soldier/Gladiator axes bring the total to **85 teaching items**. W1–W8 teach A1–A8. W2 also teaches S1; W3 combo; W4 R1; W5 S2; W6 R2. Each table entry gives its current AP; simultaneous lessons progress normally. The per-job totals below replace the old uniform 3,150-AP total.
...
|---|---:|---:|---:|---:|---:|
| Samurai | 2150 | 600 | 650 | 100 | 3500 |
| Dark Knight (each race) | 2200 | 700 | 650 | 100 | 3650 |
| Viking | 2100 | 500 | 350 | 100 | 3050 |
| Geomancer | 2200 | 500 | 650 | 100 | 3450 |
| Chemist (each race) | 2050 | 550 | 650 | 100 | 3350 |
| Bard | 1750 | 550 | 650 | 100 | 3050 |
| Dancer | 2150 | 650 | 650 | 100 | 3550 |
...
These are parallel lessons within existing shop tiers, not six stronger endgame tiers. Only Mystic Knight learns these lessons; learned actions transfer normally with a legal weapon. No extra support/reaction lessons or changes to W1–W8. All six use ordinary saber handedness, with zero other bonuses and no innate element/proc/status. Exact design gates are fixed in the acquisition ledger; ROM flag mapping and item-record capacity remain unverified.
...
### Additional council teaching weapons (0.7)

Seven repeatably obtainable parallel lessons; only the named teaching job learns each lesson, with both racial implementations sharing the Dark Knight/Chemist lesson. No new power tier, support/reaction lesson, or prerequisite. Use the acquisition ledger's exact S0–S3 gates, then verify their actual story flags and item-record capacity. Prices and AP remain tuning targets; availability of a teaching knife does not guarantee recipe ingredients.
...
Version 0.7 contains the user's adopted council foundation, Spellblade expansion, and all seven further council abilities, not an applied or gameplay-tested ROM patch. Preserve the vanilla-USA base, original jobs/abilities/stories/laws/missions/recruitment and saves, except the approved additive axe permission/lessons and explicit integration of new effects. Squire/Sentinel remain rejected and Green Mage remains a candidate.
...
Keep original unlock gates and verify any acceleration from additional mastered axe actions. Match all 85 teaching items to actual shop flags, retain repeatable teaching access without inventing consumable supply, and compare new equipment's effect on original jobs. Combo profiles require both correct initiation and chain participation. Full release needs real battle, AI, law, AP, job change, ordinary save/load, and clean-ROM patch tests; old-save migration is not assumed.

Additional required cases: ward/redirection/Poise timing; one challenger per target and per-recipient area penalties; wound snapshot/replacement/two-pulse cleanup; Refuge/Rime mutual replacement, grounded occupancy and Float; atomic medicine spending, native Cureall whitelist, Inoculation-before-Auto-Cureall, and first-application Encouragement; Passing Step before/after ordinary Move, terrain points, traps, reaction interruption, no route reroll, no extra Move. These tests are pending implementation.
...
[sam14]: https://na.finalfantasyxiv.com/jobguide/samurai/
[vik3]: https://support.na.square-enix.com/faqarticle.php?c=0&id=5141&kid=63557&la=1&page=3&pv=10&ret=faq&sc=0&so=0
[geo11]: https://support.na.square-enix.com/document/manual/20/FFXI_SeekersOfAdoulin_PC.pdf
[mixV]: https://finalfantasy.fandom.com/wiki/Mix_(Final_Fantasy_V)
[dnc14]: https://na.finalfantasyxiv.com/jobguide/dancer/
